...
@Repository
public class JdbcCoachAvailabilityRepository {
    ...
    public List<Slot> findAllByCoach(long coachId) {
        String sql = """
    ...
    public Slot insert(long coachId, int dayOfWeek, String start, String end) {
        ...
        if (hasOverlap(coachId, dayOfWeek, start, end)) {
            throw new IllegalArgumentException("Ai deja un interval suprapus in acea zi.");
        }

        String sql = """
                    insert into coach_availability (club_id, coach_id, day_of_week, start_time, end_time)
                    values (?, ?, ?, ?, ?)
                """;
    ...
    public boolean delete(long coachId, long id) {
        String sql = "delete from coach_availability where club_id=? and coach_id=? and id=?";
        try (var c = DB.getConnection();
                var ps = c.prepareStatement(sql)) {
    ...
    // existent
    private boolean hasOverlap(long coachId, int dayOfWeek, String start, String end) {
        String sql = """
                    select 1
                    from coach_availability
                    where club_id=? and coach_id=? and day_of_week=?
                      and ? < end_time
                      and ? > start_time
                    limit 1
                """;
        ...
        try (var c = DB.getConnection();
            ...
            ps.setTime(4, toSqlTime(start)); // newStart
            ps.setTime(5, toSqlTime(end)); // newEnd
